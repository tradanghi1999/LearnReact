vrpTest = {
    ...
    {
        return [timeWindowsHienTai[0] + serviceTimeHienTai, timeWindowsHienTai[1] + serviceTimeHienTai]
    },
    getKhoangThoiGianDenGap(timeWindowsHienTai, serviceTimeHienTai, timeTravelDiemTiepTheo)
    {
        let khoangThoiGianKhoiHanh =  this.getKhoangThoiGianKhoiHanh(timeWindowsHienTai,serviceTimeHienTai)
        return [
    ...
    {
        if(timeWindowsB[1] <= timeWindowsA[0] && 
            timeWindowsB[0] <= timeWindowsB[1])
            return true
        else if(timeWindowsA[1] <= timeWindowsB[0] && 
    ...
    {
        if(timeWindowsB[0] <= timeWindowsA[0] && 
            timeWindowsB[1] >= timeWindowsA[1])
            return true
        else if(timeWindowsA[0] <= timeWindowsB[0] && 
    ...
    {
        if(this.isLongNhau(timeWindowsA, timeWindowsB) == false)
            return []
        if(timeWindowsB[0] <= timeWindowsA[0] && 
            timeWindowsB[1] >= timeWindowsA[1])
            return timeWindowsA
    ...
    {
        if(this.isNgoaiNhau(timeWindowsA, timeWindowsB) == false)
            return true;
        return false
    },
    getPhanGiaoNhau(timeWindowsA, timeWindowsB)
    {
        if(this.isGiaoNhau(timeWindowsA, timeWindowsB))
        {
            ...
            else
            {
                ...
                return [Math.max.apply(null,timeStarts), Math.min.apply(null,timeEnds)]
            }
        }
        return []
    },
    getKhoangThoiGianDenGapKhachCuoiCung(timeWindows, serviceTimes, timeTravles)
    {
        ...
        {
            timeWindow = this.getKhoangThoiGianDenGap(timeWindow, serviceTimes[i], timeTravles[i])
    ...
    getThoiDiemDenDiemCuoiCung(timeWindows, serviceTimes, timeTravles){
        if(this.isGiaoNhau(
            this.getKhoangThoiGianDenGapKhachCuoiCung(timeWindows, serviceTimes, timeTravles),
    ...
    getThoiDiemDiNenBatDau(timeWindows, serviceTimes, timeTravles){
        if(timeWindows.length == 1)
            return this.randomizeThoiDiemTrongKhungGio(timeWindows[0]) - timeTravles[0]
        ...
        let thoiDiemGapKhachCuoi = this.getThoiDiemDenDiemCuoiCung(timeWindows,serviceTimes, timeTravlesNoDepot)
        let tongServiceTime = serviceTimes
                                .filter(function(x,i){
                                    if(i == serviceTimes.length - 1)
                                        return false
                                    return true
                                })
                                .reduce((a,b)=>a+b)
